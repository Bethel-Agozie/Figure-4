function [exitflag,guess] = resonance_SSoR_Vn_gen(u1,u2,k,m,n1,n2,A2bar,A1,A2,guess,A21_guess)
   exitflag = 0;
   options = optimoptions('fsolve', 'FunctionTolerance', 1e-8, 'OptimalityTolerance', 1e-8);
    ...
    c21=sqrt(k*(m*(A/A2bar).^m+n2*(A/A2bar).^(-n2)));
    u21=rRout(A);
    v=u21+c21;
    end
   ...
   % Initial celerities
    ...
    u31=u12+sqrt((m*(A11.^(m+1)-A12.^(m+1))/(m+1)+...
        n1*(A12.^(-n1+1)-A11.^(-n1+1))/(n1-1))*(A11-A12)/(A12*A11));
    ...
    sd2=(A11*u11^2-A12*u12^2+ m/(m+1)*(A11^(m+1)-A12^(m+1))+...
        n1/(n1-1)*(A12^(-n1+1)-A11^(-n1+1)))/(A11*u11-A12*u12);

    se1=(A1*u1-A11*u11)/(A1-A11);
    se2=(A1*u1^2-A11*u11^2+m/(m+1)*(A1^(m+1)-A11^(m+1))+...
        n1/(n1-1)*(A11^(-n1+1)-A1^(-n1+1)))/(A1*u1-A11*u11);
