function [exitflag,guess] = resonance_SSoR_Vm_gen(u1,u2,k,m1,m2,n,A2bar,A1,A2,guess,A21_guess)
   exitflag = 0;
   options = optimoptions('fsolve', 'FunctionTolerance', 1e-8, 'OptimalityTolerance', 1e-8);
    ...
    function u=rRout(A)
       f1=@(a) (k*(m2*a.^(m2-2)/A2bar.^m2+n*a.^(-n-2)/A2bar.^(-n))).^(1/2);
      int1=integral(f1,A2,A);
    u=u2+int1;
    end

    function v=find_A21(A)
    c21=sqrt(k*(m2*(A/A2bar).^m2+n*(A/A2bar).^(-n)));
    u21=rRout(A);
    v=u21+c21;
    end
   ...
   % Initial celerities
    ...
    function v=SSoR1(S)
        ...
        u32=u1+sqrt((m1*(A1.^(m1+1)-A11.^(m1+1))/(m1+1)+...
            ...
            v(1)=u31-u32;
            v(2)=1/2*u12^2 + A12.^m1-A12.^(-n)-1/2*u21^2-k*((A21/A2bar).^m2-(A21/A2bar).^(-n));
    end
