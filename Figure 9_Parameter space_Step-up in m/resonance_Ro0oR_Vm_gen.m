function [exitflag,guess] = resonance_Ro0oR_Vm_gen(u1,u2,k,m1,m2,n,A2bar,A1,A2,guess,A11_guess)
    ...
    u22a=A21*u21/A22;
    fun = @(A) sqrt(k*(m2*A.^(m2-2)/A2bar.^m2+n*A.^(-n-2)/A2bar.^(-n)));
    ...
    if exit1 > 0 && (u11-c11)>(u1-c1) && u12-c12>0 && u21+c21>0 && A1>=A11 && A2>=A22 && (u22b+c22) < (u2+c2) &&...
            ms<=m2 && ms>=m1 && A11<=A22 && isreal(A12) && isreal(A21) && isreal(A22)&& isreal(ms)   
   ...
   function v=Ro0oR1(S)
    ...
    u22=u22a;
        ...
        v(1)=1/2*u11^2+(A11.^m2-A11.^(-n))-1/2*u12^2-k*(A12.^ms-A12.^(-n));
        v(2)=u21a-u21b;
        v(3)=u22a-u22b;
