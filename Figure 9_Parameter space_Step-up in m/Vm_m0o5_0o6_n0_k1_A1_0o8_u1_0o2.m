...
A2bar= 1;
k=1;
m1=0.5;% Go to classical_Vm_gen.m and choose the appropriate initial guess for the classical case
m2=0.6;
n=0;
...
A11_guess = 1.05;
A12_guess = A11_guess;
guess_Ro0oS = [0.5800   0.3226   0.4227    0.6058];
guess_Ro0oR = [0.5006   0.4366   0.4366    0.6046];
guess_RoSS = [0.3079    0.3739];
guess_RoSR = [0.3079    0.3739];

A22_guess=1.3;
A21_guess=A22_guess;
guess_So0oR= [0.5177    2.1980    2.0735    1.3219];
guess_SSoR = [2.2184    1.3262];
...
for un = 1:length(u2range)
  ...
  if u2<c2 && u2>-c2  % Consider only subcritical regime
    ...
    [exitflag_Ro0oS,guess_Ro0oS] = resonance_Ro0oS_Vm_gen(u1,u2,k,m1,m2,n,A2bar,A1,A2,guess_Ro0oS,A11_guess);
    [exitflag_Ro0oR,guess_Ro0oR] = resonance_Ro0oR_Vm_gen(u1,u2,k,m1,m2,n,A2bar,A1,A2,guess_Ro0oR,A11_guess);
    [exitflag_RoSS,guess_RoSS] = resonance_RoSS_Vm_gen(u1,u2,k,m1,m2,n,A2bar,A1,A2,guess_RoSS,A12_guess);
    [exitflag_RoSR,guess_RoSR] = resonance_RoSR_Vm_gen(u1,u2,k,m1,m2,n,A2bar,A1,A2,guess_RoSR,A12_guess);
    [exitflag_So0oR,guess_So0oR] = resonance_So0oR_Vm_gen(u1,u2,k,m1,m2,n,A2bar,A1,A2,guess_So0oR,A22_guess);
    [exitflag_SSoR,guess_SSoR] = resonance_SSoR_Vm_gen(u1,u2,k,m1,m2,n,A2bar,A1,A2,guess_SSoR,A21_guess);
    [x, exitflag] = classical_Vm_gen(u1,u2,k,m1,m2,n,A2bar,A1,A2);
...
% Critical boundary
c2=sqrt(k*(m2*(A2range/A2bar).^m2+n*(A2range/A2bar).^(-n)));
hold on
plot(A2range,c2,'k')
ylim([-1 1])
plot(A2range,-c2,'k')