function [exitflag,guess] = resonance_So0oR_Vm_gen(u1,u2,k,m1,m2,n,A2bar,A1,A2,guess,A22_guess)
   ...
   % Define functions to find_A22 and rRout
    ...
    c22=sqrt(k*(m2*(A/A2bar).^m2+n*(A/A2bar).^(-n)));
    u22=rRout(A);
    v=u22+c22;
    end
 ...
 % Define the cases of the system of nonlinear equations and apply the initial guess
    ...
    u12a=u21*A21/A12;
    u12b=u21+sqrt(k*(ms*(A12.^(ms+1)-A21.^(ms+1))/((ms+1)*A2bar.^ms)+...
        n*(A21.^(-n+1)-A12.^(-n+1))/((n-1)*A2bar.^(-n)))*(A12-A21)/(A12*A21));
    u12=u12a;
    c12=sqrt(k*(ms*A12.^ms+n*A12.^(-n)));
    u11a=A12*u12/A11;
    u11b=u1+sqrt((m1*(A1.^(m1+1)-A11.^(m1+1))/(m1+1)+...
    ...
    if exit1 > 0 && (u11-c11)<lS && lS<(u1-c1) && (u22+c22) < (u2+c2) && u12-c12<0 && u21+c21<0 && A1<A11 &&...
            A2>A22 && ms<=m2 && ms>=m1 && isreal(A11) && isreal(A12) && isreal(A21)&& isreal(ms)
        disp(['exit1: ' num2str(exit1)]);
        exitflag = 1;
        guess = [ms,A11,A12,A21];
    end
